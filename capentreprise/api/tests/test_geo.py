...
url = reverse('api')
url_with_params = f"{url}?table=D_Geo"
...
date_pk_test = view(request_pk, code_PK='0184')
date_pk_test.render()
...
class Test_Vaccine(TestCase):
    ...
    def test_get_data_geo_pk(self):
        expected_result = 'Ain'
        self.assertEqual(json_response_pk['data'][0]['libelle_departement'], expected_result)
        print(json_response_pk['data'][0]['libelle_departement'])
